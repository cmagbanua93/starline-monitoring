...
// Protocol is normally inferred from the port (443/8729 -> https, else
// http). That breaks when the router sits behind a port-forward/relay
// (e.g. a remote-access provider mapping an arbitrary external port to
// the router's internal 443/www-ssl service) — the external port number
// no longer matches RouterOS's default REST ports even though the
// service on the other end is still HTTPS. Set MIKROTIK_PROTOCOL=https
// (or http) explicitly in that case; it overrides the guess below.
const PROTOCOL_OVERRIDE = String(process.env.MIKROTIK_PROTOCOL || '').toLowerCase();
const PROTOCOL          = (PROTOCOL_OVERRIDE === 'https' || PROTOCOL_OVERRIDE === 'http')
  ? PROTOCOL_OVERRIDE
  : ((MIKROTIK_PORT == 443 || MIKROTIK_PORT == 8729) ? 'https' : 'http');
