 ...
 *
 * Online/offline status and customer data (name, account number, contact
 * number, area, NAP box, plan) all come from the TaokiNinam billing system
 * (taokininam.com) — specifically its "Online PPPoE" (activesP.php) and
 * "Offline PPPoE" (inactivesP.php) pages, which TaokiNinam itself keeps in
 * sync with the Mikrotik router in real time. This means monitor.js never
 * talks to the router directly — it only needs to reach taokininam.com,
 * which works from anywhere (including cloud hosts like Railway) without
 * any VPN, port-forward, or relay.
 *
 * "Went offline at" time is not provided by TaokiNinam in bulk, so it's
 * tracked locally: the first poll where an account is seen on the offline
 * list right after being on the online list is recorded as its logout
 * time. This is accurate to within one poll interval.
 *
 ...
 *
 * The router-facing MIKROTIK_* env vars are NOT used by this file — they're
 * only needed by create_pppoe_accounts.js (the separate bulk provisioner),
 * which still talks to the router directly and should keep running on your
 * local network.
 *
 * Usage:
 *   node monitor.js
 ...
 *
 * .env options (all required except where noted):
...
// ---- TaokiNinam billing system (sole data source — status + customer info) ----
...
if (!BILLING_USERNAME || !BILLING_PASSWORD) {
  console.error('Missing TAOKININAM_USERNAME or TAOKININAM_PASSWORD in .env — these are required (monitor.js now gets all its data from TaokiNinam, not the router).');
  process.exit(1);
}
...
let state = {
  ...
  lastPoll: null,
  pollError: null,
  routerHost: `${BILLING_BASE_URL} (activesP.php / inactivesP.php)`,
...
//      already joined with billing/customer/NAP data server-side. These
//      pages are generated by TaokiNinam querying the Mikrotik router live
//      (via TaokiNinam's own VPN link to it), so which page an account
//      appears on reflects real router connection state right now — the
//      "Status: Enabled/Disabled" field inside each row is a separate
//      admin/provisioning flag, not connectivity, and should be ignored
//      for that purpose.
...
function buildAccount(fields, isOnline, now, prevByUsername) {
  const prev = prevByUsername.get(fields.username);
  const justWentOffline = !isOnline && prev && prev.status === 'online';
  const lastLogout = justWentOffline ? now.toISOString()
                    : (!isOnline && prev ? prev.lastLogout : null);
...
async function pollSubscribers() {
  try {
    ...
    const onlineFields  = parseSubscriberRows(onlineHtml).map(extractAccount).filter(Boolean);
    const offlineFields = parseSubscriberRows(offlineHtml).map(extractAccount).filter(Boolean);
    ...
    const seen = new Set();
    const updated = [];
    for (const fields of onlineFields) {
      if (seen.has(fields.username)) continue;
      seen.add(fields.username);
      updated.push(buildAccount(fields, true, now, prevByUsername));
    }
    for (const fields of offlineFields) {
      if (seen.has(fields.username)) continue;
      seen.add(fields.username);
      updated.push(buildAccount(fields, false, now, prevByUsername));
    }
    ...
    state = {
      ...
      pollError: null,
      billingLastSync: now.toISOString(),
      billingError: null,
      billingCustomerCount: updated.length,
    };

    broadcast();
  } catch (err) {
    const msg = describeFetchError(err);
    state = { ...state, pollError: msg, billingError: msg, lastPoll: new Date().toISOString() };
    broadcast();
...
║  Data source  →  ${BILLING_BASE_URL.padEnd(28)} ║
║  Polling every ${Math.round(POLL_INTERVAL_MS/1000)}s                          ║
║  Alert when ${ALERT_THRESHOLD}+ accounts offline              ║
╚══════════════════════════════════════════════╝

Status comes from TaokiNinam's activesP.php/inactivesP.php pages
(which TaokiNinam itself keeps synced with the router live).
"Logged Out At" times are detected locally the first poll an
account moves from online to offline — accurate to one poll
interval (currently ${Math.round(POLL_INTERVAL_MS/1000)}s).
`);
  pollSubscribers();
  setInterval(pollSubscribers, POLL_INTERVAL_MS);
  pollIsps();
