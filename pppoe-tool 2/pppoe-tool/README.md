...
monitor.js gets **all** of its data — online/offline status, customer name,
account number, contact number, area, and NAP box — from the TaokiNinam
billing system (taokininam.com), which already has its own connection to
your Mikrotik router and keeps those pages live. monitor.js never talks to
the router itself, so there's no VPN, port-forward, or relay to set up —
it just needs to reach taokininam.com, which works from anywhere, including
Railway.
...
3. **Set environment variables** in Railway's dashboard (Settings → Variables)
   ...
   - `TAOKININAM_USERNAME`, `TAOKININAM_PASSWORD` — **required**, same login
     you use at taokininam.com
   ...
   - Don't set `MONITOR_PORT`/`PORT` — Railway injects `PORT` automatically and
     the app binds to it.
   - You do **not** need any `MIKROTIK_*` variables here — those are only
     for `create_pppoe_accounts.js`, which you run locally.
4. **ISP uplink pings**: these ping the 5 ISP gateway IPs directly from
...
If you'd rather not put your TaokiNinam credentials on Railway, or want
the ISP uplink pings to reflect your actual site connectivity, run
`monitor.js` on an always-on machine at your site (a mini PC, an old
desktop, a Raspberry Pi) and only expose the *dashboard page* to the
internet.

1. **Use the same `.env`** as above — `TAOKININAM_USERNAME`/`PASSWORD` are
   all that's required; nothing router-specific is needed since monitor.js
   talks to TaokiNinam, not the router.
